...
from src.shared_types import GameState, Grid, Pos
...
class Gum:
    ...
    def __init__(self, screen: pg.Surface, game_layout: GameLayout) -> None:
        ...
        self.path_sprite: Path = (
            Path(__file__).resolve().parent / "sprites" / "pacgums"
        )
        self.gum_sprite: pg.Surface = pg.transform.scale(
            pg.image.load(f"{self.path_sprite}/pacgum.png").convert_alpha(),
...
class SuperGum(Animation):
    ...
    def __init__(self, screen: pg.Surface, game_layout: GameLayout) -> None:
        ...
        self.super_gum_sprites: list[pg.Surface] = [
            pg.transform.scale(
                pg.image.load(
...
class MazeView:
    ...
    def __init__(
        ...
        self.path_sprite: Path = (
            Path(__file__).resolve().parent / "sprites" / "tiles"
        )
        self.wall_sprites: list[pg.Surface] = [
            pg.transform.scale(
                pg.image.load(
                    f"{self.path_sprite}/wall_{i:02d}.png"
                ).convert_alpha(),
                (self.size, self.size),
            )
            for i in range(16)
        ...
        for row_n in range(grid_h):
            ...
            for col_n in range(grid_w):
                if row[col_n] == "wall":
                    ...
                    if (
                        row_n + 1 < grid_h
                        and self.grid[row_n + 1][col_n] == "wall"
                    ):
